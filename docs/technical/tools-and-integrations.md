...
> Chatbox Plus 是 GPLv3 fork，不内置上游 Chatbox AI 账号、订阅、license 或云端商业服务。旧版 Pro/商业服务设计仅作为上游历史背景，不代表本 fork 当前能力。

---

## 系统概览

Chatbox Plus 的工具系统为 AI 模型提供外部能力调用，使模型不仅能生成文本，还能执行搜索、读取文件、查询知识库、调用第三方服务等操作。当前工具系统由三个独立层次组成：
...
[Model Context Protocol (MCP)](https://modelcontextprotocol.io/) 是一种开放协议，允许 AI 模型通过标准化接口调用外部工具。Chatbox Plus 集成 MCP 的目的是让用户可以自由扩展 AI 的能力边界——无需修改应用代码即可接入新工具。
...
Chatbox Plus 不连接上游商业 MCP endpoint，也不依赖 license 鉴权的云端内置 MCP 服务。用户可以自行添加本地 Stdio MCP 服务器或远程 HTTP MCP 服务器。
...
工具的 `execute` 函数同时透传 `abortSignal`，使第三方搜索和读取网页请求在用户取消工具执行时能中止底层 HTTP 请求。
