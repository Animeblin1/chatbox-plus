...
Chatbox Plus 的 AI 供应商（Provider）系统负责对接多家 AI 模型服务商，包括 OpenAI、Claude、Gemini、DeepSeek、Groq、Ollama 等。该系统采用**注册表（Registry）模式**，以 `defineProvider()` 为核心实现供应商的声明式注册，是整个 AI 调用链路的入口层。

> Chatbox Plus 是 GPLv3 fork，不内置上游 Chatbox AI 账号、订阅、license 或商业远程服务。
...
模块加载时 `defineProvider()` 自动执行，将供应商写入注册表。**导入顺序决定了 UI 中供应商的显示顺序**。
...
- 未在映射表中的供应商（Ollama、LM Studio、Azure 等）不参与 models.dev 富化
- 映射关系同时被运行时富化和构建时快照生成脚本共用（**单一数据源**）

当前映射覆盖：OpenAI、Claude、Gemini、xAI、DeepSeek、Groq、Mistral、Perplexity、OpenRouter、SiliconFlow、ChatGLM 等。
...
| App Provider | OAuth Provider ID | Flow 类型 | 关键实现 |
|------|------|------|------|
