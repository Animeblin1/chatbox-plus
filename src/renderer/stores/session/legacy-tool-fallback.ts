...
import type { KnowledgeBase, Message, MessageToolCallPart } from '@shared/types'
...
function normalizeWebSearchResults(
  searchResults: Array<{ title: string; snippet: string; link: string; rawContent?: string | null }>
): WebPromptSearchResults {
...
export async function applyLegacyToolFallback(options: {
  ...
  if (kbNotSupported && webNotSupported && options.knowledgeBase) {
    ...
    if (callResult.searchResults.length && callResult.type !== 'none') {
      ...
      promptMsgs =
        callResult.type === 'knowledge_base'
          ...
          : constructMessagesWithSearchResults(promptMsgs, normalizeWebSearchResults(callResult.searchResults))
    }
    ...
    if (callResult.searchResults.length) {
      ...
      promptMsgs = constructMessagesWithSearchResults(promptMsgs, normalizeWebSearchResults(callResult.searchResults))
    }
  }
