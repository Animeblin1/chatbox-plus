...
import curlToolSet from '@/packages/model-calls/toolsets/curl'
...
import { getUtilityToolSet } from '@/packages/model-calls/toolsets/utility'
...
export async function buildToolsForSession(
  ...
  if (webSupported) {
    ...
    if (toolSettings.curl.enabled) {
      instructions += curlToolSet.description
    }
  }
  if (utilityToolSet.description) {
    instructions += utilityToolSet.description
  }
  ...
  tools = { ...tools, ...utilityToolSet.tools }
