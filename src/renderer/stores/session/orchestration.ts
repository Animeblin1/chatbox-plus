...
export async function orchestrateGeneration(
  ...
  try {
    ...
    if (
      ...
      promptMsgs.some((m) => m.contentParts.some((part) => part.type === 'image' && !part.ocrResult))
    ) {
    ...
    const { promptMsgs: fallbackPromptMsgs, fallbackToolCallPart } = await applyLegacyToolFallback({
      model,
      promptMsgs,
      knowledgeBase,
      webBrowsing,
    ...
    for await (const chunk of stream) {
      const result = await processStreamChunk(chunk, processorState, {
        onFileReceived: async (mediaType, base64) => {
